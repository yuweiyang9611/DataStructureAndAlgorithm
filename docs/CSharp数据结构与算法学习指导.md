---
title: C# 数据结构与算法学习指导
description: 从复杂度、不变量和测试出发，按八个阶段系统学习本仓库的数据结构、算法与综合项目。
search: false
---
...
项目以类库为核心，并提供职责单一的算法 Demo、P3 追踪 Demo、基准项目和四个综合场景项目。Demo 用 JSON/Mermaid 展示状态，Benchmarks 用独立进程测量时间和分配；综合项目把多个已学结构组合成端到端应用，并通过统一、可选的追踪 sink 解释流水线。学习时先读源码与测试，再用 Demo 观察单步状态，最后进入配送、搜索、调度和存储场景解释“为什么选这些结构、它们如何共同维护不变量”；不要把输出、计时或 UI 逻辑塞回算法类。四个场景的目标函数、复杂度、限制与正确性证据见 [综合项目实战学习指导](https://yuweiyang9611.github.io/DataStructureAndAlgorithm/综合项目实战学习指导.html)。

完成本篇主干内容后，继续阅读 [C# 数据结构与算法进阶学习指导](https://yuweiyang9611.github.io/DataStructureAndAlgorithm/CSharp数据结构与算法进阶学习指导.html)，学习哈希表、并查集、Trie、LRU、区间结构和进阶算法。

需要按经典题型练习时，阅读 [LeetCode 100 题完整学习指导](https://yuweiyang9611.github.io/DataStructureAndAlgorithm/LeetCode100题完整学习指导.html)。

新增模块应遵循 [API 设计与比较器约定](https://yuweiyang9611.github.io/DataStructureAndAlgorithm/API设计与比较器约定.html)，其中集中说明空值、半开区间、比较器、集合重复、输入所有权和异常类型为什么这样统一。
...
完整八周路线和手算练习见 [综合项目实战学习指导](https://yuweiyang9611.github.io/DataStructureAndAlgorithm/综合项目实战学习指导.html)。

### 第八阶段：持续正确性与有证据的优化 {#main-stage-8}
...
覆盖率只说明哪些代码被执行，变异分数只说明现有变体是否被测试识别，基准只说明性能；三者都不能替代领域契约和独立 oracle。完整证据矩阵见 [综合项目实战学习指导](https://yuweiyang9611.github.io/DataStructureAndAlgorithm/综合项目实战学习指导.html)。

## 8. 建议的练习方式 {#main-practice}
...
## 9. P0-P4、综合应用完成度与后续路线 {#main-progress}
...
完整推导、源码索引、逐题解释和八周计划见 [P3 数论、位运算与高级算法学习指导](https://yuweiyang9611.github.io/DataStructureAndAlgorithm/P3数论位运算与高级算法学习指导.html)。
...
这一阶段的重点不是记住四套代码，而是练习从目标函数和领域约束选择结构：先定义跨模块不变量，再让每个结构只承担自己擅长的职责，最后用端到端证据同时验证结果、最优目标、依赖、容量和恢复等约束。详细场景、复杂度、限制和扩展练习集中在 [综合项目实战学习指导](https://yuweiyang9611.github.io/DataStructureAndAlgorithm/综合项目实战学习指导.html)。
